...
fresh_install_node() {
    ...
    if [ -n "$SOLANA_PUBKEY" ] && [ -f "$HOME/.pipe-cli.json" ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            cp "$HOME/.pipe-cli.json" "$HOME/.pipe-cli.json.bak" 2>/dev/null
            echo -e "${GREEN}✅ Backup updated at ~/.pipe-cli.json.bak${NC}"
        else
...
import_old_account() {
    ...
    jq -n --arg un "$username" --arg uid "$user_id" --arg uak "$user_app_key" --arg sp "$solana_pubkey" \
          --arg at "$access_token" --arg rt "$refresh_token" --arg tt "$token_type" --arg ei "$expires_in" --arg ea "$expires_at" \
          '{username: $un, user_id: $uid, user_app_key: $uak, solana_pubkey: $sp, auth_tokens: {access_token: $at, refresh_token: $rt, token_type: $tt, expires_in: $ei | tonumber, expires_at: $ea}}' > "$HOME/.pipe-cli.json"
    if [ $? -eq 0 ]; then
